...
test('getReceivingAddress & ', async () => {
  {
    ...
    const _ethService = new ethereumService()

    // mock api response
    _ethService._HTTPAgent.get = () => ({
      success: true,
      data: {
        address
      }
...
test('addToken', async () => {
  {
    ...
    // mock db
    _ethService._DBOperator.currencyDao = {
      insertCurrency: () => {}
    }
    _ethService._DBOperator.accountCurrencyDao = {
      insertAccount: () => {}
    }
